...
%% Light Comfort Index
[l,b] = size(y);
Light_comfort = zeros(l,b);
D = zeros(l,b);
for i = 1:l*b
    [Light_comfort(i),D(i)] = Light_index(y(i));
end
%% Light Comfort level based on standards
figure
a = jet(10000);
J = heatmap(Light_comfort);
c = colormap(a);
title('Light Comfort level based on BCA,standards of the room FEC2')
caxis([0 100])
...
surf(D)
colormap(bl)
title('Light Availabilty at different points')
caxis([-2 2])
