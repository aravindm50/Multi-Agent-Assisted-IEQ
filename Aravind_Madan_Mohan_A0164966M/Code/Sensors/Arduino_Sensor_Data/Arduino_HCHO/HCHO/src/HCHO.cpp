...
HCHO::HCHO(HardwareSerial* Serial)	//read the uart signal by hardware uart,such as D0
{
    mySerial = Serial;
    receivedFlag = 0;
}

HCHO::HCHO(ParticleSoftSerial* Serial)	//read the uart signal by software uart,such as D10
{
    mySerial = Serial;
    receivedFlag = 0;
}
HCHO::HCHO(int pin,float ref)//read the analog signal by analog input pin ,such as A2; ref:voltage on AREF pin
{
    _sensorPin = pin;
    _ref = ref;											//for arduino uno ,the ref should be 5.0V(Typical)
}

byte HCHO::checkSum(byte array[],byte length)	
{
    byte sum = 0;
...
boolean HCHO::available()		//new data was recevied
{
